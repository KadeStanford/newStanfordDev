...
function initAdmin() {
  ...
  console.log(
    "Firebase Admin init: env present =>",
    { hasFirebaseSA, hasGaSaKey, hasGaSaBase64 }
  );
  ...
  if (hasGaSaBase64) {
    try {
      ...
      console.error("Failed to parse GA_SA_KEY_BASE64 service account JSON:", e);
    }
  }
  ...
  if (sa) {
    try {
      ...
      console.error("Failed to initialize admin SDK with parsed service account:", e);
      admin.initializeApp();
    }
  } else {
