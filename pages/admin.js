...
export default function AdminDashboard() {
  ...
  useEffect(() => {
    if (!loading) {
      // FIX 1: If user exists and is not admin, push to the routing hub
  ...
  const fetchUsers = async () => {
    try {
      const q = query(collection(db, "users"), orderBy("createdAt", "desc")); // Fetching by creation time
  ...
  // FIX 2: Render loading screen if loading OR if user is loaded but not admin (waiting for redirect)
